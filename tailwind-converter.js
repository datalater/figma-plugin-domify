...
exports.cssToTailwindClasses = cssToTailwindClasses;
exports.parseCssString = parseCssString;
exports.cssStringToTailwind = cssStringToTailwind;
...
function findNearestColor(hex) {
    ...
    for (const [paletteHex, colorName] of Object.entries(tailwind_config_1.COLOR_PALETTE)) {
        const paletteRgb = hexToRgb(paletteHex);
        if (!paletteRgb)
            continue;
        const distance = Math.sqrt(Math.pow(targetRgb.r - paletteRgb.r, 2) +
...
function cssToTailwind(prop, value) {
    ...
    if (['top', 'right', 'bottom', 'left'].includes(normalizedProp)) {
        if (normalizedValue === '0px' || normalizedValue === '0')
            return `${normalizedProp}-0`;
        if (normalizedValue === 'auto')
            return `${normalizedProp}-auto`;
...
function spacingClass(prefix, value) {
    ...
    const scale = tailwind_config_1.SPACING_SCALE[absoluteValue];
    if (scale) {
 ...
 */
function colorClass(prop, value) {
    const colorPrefix = prop === 'background-color' ? 'bg' : prop === 'color' ? 'text' : 'border';
    const normalizedValue = value.toLowerCase();
    // Try direct palette lookup first (hex colors)
    let colorName = tailwind_config_1.COLOR_PALETTE[normalizedValue];
    if (colorName) {
    ...
    if (hex) {
        colorName = tailwind_config_1.COLOR_PALETTE[hex.toLowerCase()];
        if (colorName) {
            return `${colorPrefix}-${colorName}`;
        }
...
function opacityClass(value) {
    ...
    return tailwind_config_1.OPACITY_SCALE[opacityKey] ? `opacity-${tailwind_config_1.OPACITY_SCALE[opacityKey]}` : null;
}
/**
 * Convert border-radius value to Tailwind class
 */
function borderRadiusClass(value) {
    const scale = tailwind_config_1.BORDER_RADIUS_SCALE[value];
    if (scale) {
...
function borderWidthClass(value) {
    const scale = tailwind_config_1.BORDER_WIDTH_SCALE[value];
    if (scale) {
...
function fontSizeClass(value) {
    const scale = tailwind_config_1.FONT_SIZE_SCALE[value];
    if (scale) {
        return `text-${scale}`;
    }
    return `text-[${value}]`;
 ...
 */
function fontWeightClass(value) {
    const scale = tailwind_config_1.FONT_WEIGHT_SCALE[value];
    if (scale) {
        return `font-${scale}`;
    }
    return `font-[${value}]`;
...
function lineHeightClass(value) {
    const scale = tailwind_config_1.LINE_HEIGHT_SCALE[value];
    if (scale) {
 ...
 */
function cssToTailwindClasses(cssProperties) {
    const classes = [];
 ...
 */
function cssStringToTailwind(cssText) {
